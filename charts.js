function init() {
  ...
  function buildCharts(sample) {
    ...
    d3.json("samples.json").then((data) => {
      ...
      var filtered_id = sample_array.filter(data => data.id == sample);
      ...
      var filtered_mdata = data.metadata.filter(data => data.id == sample);
  
      // 2. Create a variable that holds the first sample in the metadata array.
      var sample_mdata = filtered_mdata[0];
  
      // 3. Create a variable that holds the washing frequency.
      var washing_freq = sample_mdata.wfreq;
